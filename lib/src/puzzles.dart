...
List<Input> parsedPuzzle(int i, List<List<String>> puzzleList) {
  if (i >= puzzleList.length) return [];
  final puzzle = <List<int>>[];
  for (var row in puzzleList[i]) {
    puzzle.add(row.runes.map((r) => int.parse(String.fromCharCode(r))).toList());
  }
  final inputs = <Input>[];
  scan((y, x) {
    if (puzzle[y][x] > 0) {
      inputs.add(Input.cursor(column: y, row: x));
